...
* 算法流程：
    * 1.遍历数组找到一个最大的值 maxValue,记录其位数为 maxValueLength   
        * 例如，数组中最大值为 1234， 则 maxValueLength = 4
    * 2.根据要排序的数字的进制n，创建大小为n的数组  
        * 如，十进制，创建数组大小为10  
    * 3.循环maxValueLength，每一轮对**当前所有数组的当前位**进行一次**计数排序**  
        * 也可以不使用计数排序  
    * 4.每一次计数排序完城后，将计数排序完的结果写回原数组  
    * 5.最后大循环结束后，排序完成  
    
* 稳定性：稳定
    * 对每一位使用计数排序时要使用稳定计数排序 **否则无法得到正确结果**
        * 例如 {25， 13， 24}，第一轮排序完
    * 
