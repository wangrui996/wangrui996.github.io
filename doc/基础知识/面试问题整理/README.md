...
* 为什么这么做呢？ 直接调用B不就行了？————任务简答时确实是这样，但是如果有很多这样的自定义函数B,C,D,E等，我们在每次调用它们之前，都会执行一段相同的代码M，执行它们之后，也都会执行一段相同的代码N, 面对这样的场景，首先我们肯定想要复用代码段M和N，那我们就将它们分别写进函数X, Y 好，这样假设我们依次需要执行B,C,D,E，那代码就是 X,B,Y,X,C,Y,X,D,Y,X,E,Y  如果我们修改下BCDE函数，即把代码段分别加入到它们的函数头和尾，这样调用BCDF即可，但是这部分代码太冗余
